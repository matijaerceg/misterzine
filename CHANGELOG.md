...
- Release tracker: the Source menu now lists databases biggest first, by the number of rows each one contributes (the count shown next to the name), instead of a fixed order.
- Release tracker: no two rows share a title any more. When a Patreon-gated row would read exactly like a public one (Coin-Op Collection's Black Heart next to kuzecores' free Black Heart), the gated row now carries its database's name, Black Heart (Coin-Op Collection), and the public row is left alone. The 1983 arcade maze game Saturn is now Saturn (Zilec), as its MRA is named, so it no longer reads like the Sega Saturn core. The feeds also match an update to its exact row (database plus file) rather than by title, so two databases shipping the same game name can no longer swap items.
...
- Site header, every page: the links (changelog, Discord, traffic, privacy, the tracker's RSS feeds and its What's listed note), the theme picker and the tracker's My MiSTer(s) button now live in one Menu button; the nav pills use the condensed face; every tooltip on the site is now the site's own popover instead of the browser's. The tracker's status line sits under the page title, when the data was last updated and what was added or updated since your last visit, and is no longer a row inside the table; the logo spans the title and that line. Renames: the tracker heading is MiSTer Core Tracker & Launcher, the Frontend page is Arcade Frontend, and the landing cards are Core Tracker, Arcade Frontend and Hardware Landscape. The Frontend menu links its own changelog and GitHub issues, the Hardware menu its own Discord thread.
