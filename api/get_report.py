"""Fetch, list or delete the reports players send from MisterZine.
    ...
    python api/get_report.py 7K2Q-9XMB        print the report and keep a copy
    python api/get_report.py --list           the reports of the last 30 days
    python api/get_report.py --delete 7K2Q-9XMB
...
The service deletes every report 30 days after it arrives.
"""
import argparse
import json
import os
...
def code_of(s):
    ...
    if not re.fullmatch(r'[0-9A-HJKMNP-TV-Z]{8}', c):
...
def main():
    ...
    p.add_argument('code', nargs='?', help='the code the player posted, e.g. 7K2Q-9XMB')
